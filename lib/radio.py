...
class Radio:
    ...
    def __init__(self, freqBand, nodeID, networkID=100, **kwargs):
        ...
        self.csPin = kwargs.get('csPin', 24 if self.spiDevice == 0 else 26)  # spi device 0 -> pin 24
        self.spyMode = kwargs.get('spyMode', 0)
        self.powerLevel = kwargs.get('power', 23)
        ...
        self.set_power_level(kwargs.get('power', 23))
    ...
    def set_power_level(self, level):
        ...
        Args:
            level (int): Value between 0 and 23.
        """
        if level > 23:
            level = 23
        ...
        # enable PA1+PA2
            ...
            level += 10 if self.powerLevel < 20 else 8
            pa_setting = RF_PALEVEL_PA1_ON | RF_PALEVEL_PA2_ON

        self._setHighPowerRegs(True)
        self._writeReg(REG_PALEVEL, pa_setting | level)
    ...
    def _sendFrame(self, toAddress, buff, requestACK, sendACK):
        ...
        with self._spiLock:
            self.spi.xfer([
                ...
                self.address & 0xff,
                ctl_byte
            ] + [i if isinstance(i, int) else int(ord(i)) for i in list(buff)])

        with self._sendLock:
            self._setMode(RF69_MODE_TX)

            now = time.time()
            while (self._readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PACKETSENT) == 0x00 and ((time.time() - now) < 10):
                pass
    ...
    def _setHighPower(self, onOff):
        self._writeReg(REG_OCP, RF_OCP_OFF)  # disable OverCurrentProtection for HW/HC
        self.set_power_level(self.powerLevel)

    def _setHighPowerRegs(self, onOff):
        onOff = False if self.powerLevel < 20 else onOff
    ...
    def _shutdown(self):
        ...
        self.sleep()
        GPIO.cleanup([self.intPin, self.rstPin, self.csPin])
    ...
    def _interruptHandler(self, pin):  # pragma: no cover
        ...
        with self._modeLock:
            ...
            if self.mode == RF69_MODE_RX and self._readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PAYLOADREADY:
                ...
                with self._spiLock:
                    payload_length, target_id_h, target_id_m, target_id_l, sender_id_h, sender_id_m, sender_id_l, CTLbyte = self.spi.xfer2([REG_FIFO & 0x7f, 0, 0, 0, 0, 0, 0, 0, 0])[1:]
                    target_id = (target_id_h << 16) | (target_id_m << 8) | target_id_l
                    sender_id = (sender_id_h << 16) | (sender_id_m << 8) | sender_id_l
                ...
                if not ack_received:
                    ...
                    with self._packetLock:
                        self._packets.append(
                            Packet(int(target_id), int(sender_id), int(rssi), list(data), ack_requested)
                        )
            ...
            with self._spiLock:
                if isinstance(buff, str):
                    ...
                    self.spi.xfer2([
                        ...
                        (self.address >> 8) & 0xff,
                        self.address & 0xff,
                        timeRemaining & 0xFF, (timeRemaining >> 8) & 0xFF
                    ] + buff)
